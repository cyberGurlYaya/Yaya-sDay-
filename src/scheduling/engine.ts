...
export function buildDayPlan(tasks: Task[], date = new Date()): ScheduledBlock[] {
  ...
  for(const task of flexible){
    ...
    let conflict=result.find(block=>start<new Date(block.scheduledEnd)&&end>new Date(block.scheduledStart));

    if(conflict){
      cursor=Math.max(cursor+10,new Date(conflict.scheduledEnd).getHours()*60+new Date(conflict.scheduledEnd).getMinutes());
      start=minutesToDate(date,cursor);
      end=new Date(start.getTime()+duration*60000);
      conflict=result.find(block=>start<new Date(block.scheduledEnd)&&end>new Date(block.scheduledStart));
    }
...
function splitBrainDump(input: string) {
  ...
  const chunks=normalized
    .split(/\s*[,;\n]\s*|\s+(?:and then|then|after that|also)\s+/i)
    .map(s=>s.trim())
    .filter(Boolean);
...
function extractTime(text:string,baseDate:Date,kind:Task['kind']){
  ...
  if(!/(?:at|around|@|by)/i.test(match[0]))return undefined;
  ...
  // A "by" time is a hard target for fixed commitments; for flexible work it remains a deadline.
