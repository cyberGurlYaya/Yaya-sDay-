...
export function buildDayPlan(tasks: Task[], date = new Date()): ScheduledBlock[] {
  ...
  for (const task of fixed) {
    const start = new Date(task.startsAt!); const duration = Math.max(10, task.durationMinutes ?? 30);
  ...
  for (const task of flexible) {
    ...
    let start = minutesToDate(date, cursor); let end = new Date(start.getTime()+duration*60000);
    const conflict = result.find(block => start < new Date(block.scheduledEnd) && end > new Date(block.scheduledStart));
    if (conflict) { cursor = Math.max(cursor + 10, new Date(conflict.scheduledEnd).getHours()*60 + new Date(conflict.scheduledEnd).getMinutes()); start=minutesToDate(date,cursor); end=new Date(start.getTime()+duration*60000); }
...
export function parseNaturalTaskText(input: string, baseDate = new Date()): Partial<Task>[] {
  ...
  return pieces.map(title => {
    ...
    const priority: Task['priority'] = /(must|urgent|important|deadline|absolutely|critical)/i.test(title) ? 'high' : 'medium';
    ...
    return { title, kind, priority, durationMinutes, status:'inbox' as const, ...(time ? { startsAt:time } : {}) };
  });
}

function extractTime(text: string, baseDate: Date) {
  const match=text.match(/(?:at|by|around|@)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/i);
