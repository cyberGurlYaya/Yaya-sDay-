...
function splitBrainDump(input: string) {
  ...
  const chunks = normalized.split(/\s*[,;\n]\s*|\s+(?:and then|then|after that|also)\s+/i).map(s => s.trim()).filter(Boolean);
...
export function parseNaturalTaskText(input:string, baseDate=new Date()):Partial<Task>[] {
  return splitBrainDump(input).map(title=>{
    ...
    const kind:Task['kind']=/(pray|salah|fajr|dhuhr|asr|maghrib|isha|appointment|meeting|class|shift)/i.test(title)?'fixed':/(rest|sleep|break|eat|breakfast|lunch|dinner|bath|shower|nap)/i.test(title)?'self-care':'flexible';
    const priority:Task['priority']=/(critical|emergency)/i.test(title)?'critical':/(must|urgent|important|deadline|absolutely|asap|before \w+)/i.test(title)?'high':/(whenever|if i can|maybe|optional)/i.test(title)?'low':'medium';
    ...
    const time=extractTime(title,baseDate);
    const deadline=extractDeadline(title,baseDate);
    const cleanTitle=title.replace(/\s+(?:at|by|around|@)\s*\d{1,2}(?::\d{2})?\s*(?:am|pm)?/i,'').replace(/\s+by\s+(?:today|tonight|tomorrow|\d{1,2}(?::\d{2})?\s*(?:am|pm)?)/i,'').replace(/\s+/g,' ').trim();
    return {title:cleanTitle || title,kind,priority,durationMinutes,status:'inbox' as const,...(time?{startsAt:time}:{}),...(deadline?{deadline}: {})};
  });
}

function parseClock(hourText:string, minuteText:string|undefined, meridiem:string|undefined, context:string) {
  let hour=Number(hourText); const minute=Number(minuteText||0); const mer=meridiem?.toLowerCase();
  if(mer==='pm'&&hour<12)hour+=12; if(mer==='am'&&hour===12)hour=0;
  if(!mer){if(/\b(morning|dawn|early)\b/.test(context)&&hour===12)hour=0;else if(/\b(afternoon|evening|night)\b/.test(context)&&hour<12)hour+=12;else if(hour<=6)hour+=12;}
  return {hour,minute};
}

function extractTime(text:string,baseDate:Date){
  const match=text.match(/(?:at|around|@)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/i); if(!match)return undefined;
  const {hour,minute}=parseClock(match[1],match[2],match[3],text.toLowerCase());
...
function extractDeadline(text:string,baseDate:Date){
  const match=text.match(/\bby\s+(today|tonight|tomorrow|(?:\d{1,2})(?::(\d{2}))?\s*(?:am|pm)?)/i); if(!match)return undefined;
  const value=match[1].toLowerCase(); const d=new Date(baseDate);
  if(value==='tomorrow') d.setDate(d.getDate()+1);
  if(value==='tonight') d.setHours(21,0,0,0);
  else if(value==='today') d.setHours(23,59,0,0);
  else { const clock=value.match(/(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/i); if(clock){const parsed=parseClock(clock[1],clock[2],clock[3],text.toLowerCase()); d.setHours(parsed.hour,parsed.minute,0,0);} }
