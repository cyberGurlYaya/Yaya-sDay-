...
function voiceLabel(voice: DeviceVoice) { return voice.name?.trim() || voice.identifier.split(/[-#]/)[0] || 'English voice'; }
...
export default function Settings() {
  ...
  useEffect(() => {
    Speech.getAvailableVoicesAsync().then(all => {
      const english = all.filter(v => /^en(-|$)/i.test(v.language));
      const sorted = [...english].sort((a,b) => {
        const aScore = /female|woman|samantha|ava|jenny|aria|karen|moira|google us english/i.test(`${a.name} ${a.identifier}`) ? 0 : 1;
        const bScore = /female|woman|samantha|ava|jenny|aria|karen|moira|google us english/i.test(`${b.name} ${b.identifier}`) ? 0 : 1;
        return aScore - bScore;
      });
      setVoices(sorted.slice(0, 6));
    }).catch(() => undefined);
  }, []);
  ...
  const previewVoice = (voice: DeviceVoice) => {
    saveProfile({ voiceId: voice.identifier });
    Speech.stop();
    Speech.speak(`Hi ${profile.nickname || 'there'}, I’m Yaya. Let’s make today feel doable.`, { voice: voice.identifier, language: voice.language, rate: 0.93, pitch: 1.02 });
  };
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Section title="YAYA'S VOICE" icon="🎙️">
      <Text style={styles.sectionHint}>Pick a voice you actually enjoy hearing. Tap one to preview it and make it Yaya's voice.</Text>
      {voices.length ? voices.map(voice => <Pressable key={voice.identifier} onPress={() => previewVoice(voice)} style={[styles.voiceChoice, profile.voiceId===voice.identifier && styles.selected]}><View style={{flex:1}}><Text style={styles.personalityText}>{voiceLabel(voice)}</Text><Text style={styles.copy}>{voice.language}{voice.quality ? ` · ${voice.quality}` : ''}</Text></View><Text style={styles.play}>{profile.voiceId===voice.identifier ? '✓' : '▶'}</Text></Pressable>) : <Text style={styles.sectionHint}>No English system voices were reported by this device yet.</Text>}
    </Section>
